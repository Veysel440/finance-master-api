...
func (r *TxRepo) List(userID int64, page, size int, q string) ([]ports.Transaction, int, error) {
	...
	err := r.db.Select(&rows, `
		...
		FROM transactions t `+where+` ORDER BY occurred_at DESC, id DESC LIMIT ? OFFSET ?`, args2...)
...
func (r *TxRepo) Create(userID int64, t *ports.Transaction) error {
	res, err := r.db.Exec(`
		INSERT INTO transactions (user_id, wallet_id, category_id, type, amount, currency, note, occurred_at, updated_at)
		VALUES (?,?,?,?,?,?,?,?,NOW())`,
...
func (r *TxRepo) Update(userID int64, t *ports.Transaction) error {
	_, err := r.db.Exec(`
		UPDATE transactions SET wallet_id=?, category_id=?, type=?, amount=?, currency=?, note=?, occurred_at=?, updated_at=NOW()
...
func (r *TxRepo) SoftDelete(userID int64, id int64) error {
	_, err := r.db.Exec(`UPDATE transactions SET deleted_at=NOW(), updated_at=NOW() WHERE id=? AND user_id=?`, id, userID)
	return err
}
...
func (r *TxRepo) UpsertBatch(userID int64, items []ports.Transaction) error {
	...
	for i := range items {
		...
		if _, err = tx.Exec(ins,
			...
			it.OccurredAt, it.UpdatedAt, it.DeletedAt); err != nil {
			return err
		}
	}
	return nil
